...
class ParsedLedgerCache:
    ...
    async def invalidate(self, snapshot: RepositorySnapshot) -> None:
        def matches(key: LedgerCacheKey) -> bool:
            return (
                key.sha == snapshot.sha
                and key.entrypoint == snapshot.entrypoint
                and key.root == snapshot.root.resolve()
            )

        async with self._lock:
            for key in [key for key in self._values if matches(key)]:
                self._values.pop(key, None)
            inflight = [task for key, task in self._inflight.items() if matches(key)]
        if inflight:
