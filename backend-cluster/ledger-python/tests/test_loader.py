...
from bea_engine.managed_load import LoadedLedger as ManagedLoadedLedger
from bea_engine.managed_load import IncludeRef, ManagedSource
from app import repository
from app import ledger
from app.errors import ServiceError
from app.ledger import load_snapshot, plugin_names, read_managed_price_sources
...
def test_managed_price_sources_use_public_fields_and_force_refresh(
    tmp_path: Path,
    monkeypatch,
) -> None:
    (tmp_path / "main.bean").write_text("", encoding="utf-8")
    ...
    source = ManagedSource(
        url=url,
        alias="BTC-USD",
        included_from=(IncludeRef(file="main.bean", line=1, target=url),),
        commodity="BTC",
        quote="USD",
        source="provider",
        revision="r1",
        etag='"r1"',
        observed_at="2026-09-15T08:25:00Z",
        fetched_at="2026-09-15T08:26:00Z",
        next_refresh_at="2026-09-15T08:35:00Z",
        freshness="recent",
        error=None,
        shadowed_count=1,
        effective_dates=("2026-09-15",),
    )
    refreshed: list[str] = []
    monkeypatch.setattr(ledger, "zero_next_refresh", refreshed.append)
    monkeypatch.setattr(
        ledger,
        "load_with_sources",
        lambda _entry: ManagedLoadedLedger([], [], {}, (source,)),
    )

    result = read_managed_price_sources(
        snapshot(tmp_path), refresh_urls=(url, url)
    )
    ...
    assert result == [
        {
            ...
            "alias": "BTC-USD",
            "includedFrom": [
                {"file": "main.bean", "line": 1, "target": url}
            ],
            "commodity": "BTC",
            "quote": "USD",
            "source": "provider",
            ...
            "fetchedAt": "2026-09-15T08:26:00Z",
            "nextRefreshAt": "2026-09-15T08:35:00Z",
            "freshness": "recent",
            "error": None,
            "shadowedCount": 1,
        }
