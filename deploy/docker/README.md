...
4. Enable Managed OAuth and dynamic client registration for the Beancount
  Access application, and enable **Allow loopback clients** for the macOS Git
  helper's `127.0.0.1` callback. The helper discovers Cloudflare's
  protected-resource metadata and dynamically registers a public PKCE client;
  it does not store a client secret.
...
Browser traffic uses the Access cookie.
